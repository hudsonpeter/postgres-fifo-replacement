...
#define MAX_BUFFER_SIZE_FIFO 16384
typedef struct {
	int front;
	int rear;
	Buffer buffers[MAX_BUFFER_SIZE_FIFO];
} FifoQueue;

FifoQueue *fifoQueue = NULL;
static void initializeFifoQueue() {
    bool found;
    fifoQueue = (FifoQueue *) ShmemInitStruct(
        "FIFO Queue Init", sizeof(FifoQueue), &found);
    if (fifoQueue == NULL) {
        elog(ERROR, "space unavailable for fifoQueue");
    }
...
static void enqueueBuffer(BufferDesc *buf) {
    uint32 local_buf_state;
    
    if ((fifoQueue->rear + 1) % MAX_BUFFER_SIZE_FIFO == fifoQueue->front) {
        elog(WARNING, "FIFO queue is full, cannot enqueue buffer");
        return;
    }
    ...
    if (BUF_STATE_GET_REFCOUNT(local_buf_state) == 0 && 
        ...
        fifoQueue->rear = (fifoQueue->rear + 1) % MAX_BUFFER_SIZE_FIFO;
        fifoQueue->buffers[fifoQueue->rear] = BufferDescriptorGetBuffer(buf);
    } else {
        UnlockBufHdr(buf, local_buf_state);
    }
}

static Buffer dequeueBuffer() {
	Buffer buf;
    if (fifoQueue->front == (fifoQueue->rear + 1) % MAX_BUFFER_SIZE_FIFO) {
        elog(WARNING, "FIFO queue is empty, cannot dequeue buffer");
        return InvalidBuffer;
    }
    
    buf = fifoQueue->buffers[fifoQueue->front];
    fifoQueue->front = (fifoQueue->front + 1) % MAX_BUFFER_SIZE_FIFO;
    return buf;
}

static BufferDesc *GetVictimBuffer() {
    BufferDesc *buf;
    uint32 local_buf_state;
	Buffer victimBuffer;
    ...
    if (fifoQueue->front == (fifoQueue->rear + 1) % MAX_BUFFER_SIZE_FIFO) {
        elog(WARNING, "No victim buffer available in FIFO queue.");
        return NULL;
    }

    victimBuffer = fifoQueue->buffers[fifoQueue->front];
    buf = GetBufferDescriptor(victimBuffer);
...
static BufferDesc *GetNewBuffer() {
    BufferDesc *buf = NULL;
	uint32 local_buf_state;
    
    for (int i = 0; i < MAX_BUFFER_SIZE_FIFO; i++) {
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	if (StrategyControl->enable_fifo) {
		//elog(LOG, "Using FIFO Strategy");
		if (fifoQueue == NULL) {
			initializeFifoQueue();
		}
		...
		if (buf) {
			...
			if (fifoQueue->front != (fifoQueue->rear + 1) % MAX_BUFFER_SIZE_FIFO) {
				buf = GetVictimBuffer();
				if (!buf) {
					elog(WARNING, "No available buffers for replacement.");
					return InvalidBuffer;
				}
			} else {
				elog(WARNING, "FIFO queue is empty, no buffers to reuse.");
				return InvalidBuffer;
			}
		}
		//elog(LOG, "Returning from FIFO Strategy");
		return buf;
	}

	//elog(LOG, "Using LRU(Default) Strategy");
	*from_ring = false;
...
StrategyInitialize(bool init)
{
	...
	if (!found)
	{
		...
		// enable fifo
		StrategyControl->enable_fifo = false;
	}
	else
		Assert(!init);
}
